...
const BubbleDonutChart: FunctionComponent<BubbleDonutChartProps> = ({
  ...
  useLayoutEffect(() => {
    if (null !== svgWrapperRef.current) {
      ...
      d3BubbleSelectionRef.current = createD3SvgRoot(svgWrapperRef.current, size);
  ...
  useEffect(() => {
    function handleResize() {
      if (null !== svgWrapperRef.current && null != d3BubbleSelectionRef.current) {
        const newSize = Math.min(svgWrapperRef.current.clientWidth, svgWrapperRef.current.clientHeight);
