...
export function layoutBubbles(
    ...
    const d3BubbleSelection = selection.selectAll<SVGCircleElement, BubbleWithCoordsAndRadius>("circle").data(d3Bubbles, (d) => (d as BubbleWithCoordsAndRadius).id);

    d3BubbleSelection.enter()
        .append("circle")
        .attr("opacity", 0)
 ...
 * @param size
 */
export const resizeD3ViewportAndBubbles = (d3SvgRoot: D3SVGSelection, size: number) => {
    d3SvgRoot
        .attr("height", size)
        .attr("width", size);
    ...
    d3SvgRoot.selectAll<SVGCircleElement, BubbleWithCoordsAndRadius>("circle")
        .attr("cx", (d) => viewportScale(d.x + DONUT_CENTER))
        .attr("cy", (d) => viewportScale(d.y + DONUT_CENTER))
        .attr("r", (d) => viewportScale(d.r))
    ;
