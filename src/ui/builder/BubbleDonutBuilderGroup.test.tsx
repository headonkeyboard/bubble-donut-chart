...
describe('BubbleDonutBuilderGroup', () => {
    test('BubbleDonutBuilderGroup should display a slider input with a label', () => {
        const component = renderer.create(
            <BubbleDonutBuilderGroup index={1} defaultSize={100} color="#EEEEEE" onChange={() => {}} />
        );
    ...
    test('BubbleDonutBuilderGroup onChange function should receive new slider value after each slider change', () => {
        ...
        const {container} = render(
            <BubbleDonutBuilderGroup index={GROUP_INDEX} defaultSize={SLIDER_INITIAL_VALUE} color="#EEEEEE" onChange={onChangeCallback} />
        );
        ...
        expect(onChangeCallback).toHaveBeenCalledWith(GROUP_INDEX, TRIGGER_SLIDER_NEW_VALUE);
    });
});
