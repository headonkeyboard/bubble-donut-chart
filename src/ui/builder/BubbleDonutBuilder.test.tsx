...
describe("BubbleDonutBuilder", () => {
  test("BubbleDonutBuilder should display a BubbleDonutBuilderGroup for each group sizes", () => {
    const component = renderer.create(
      <BubbleDonutBuilder
        groupSizes={INITIAL_GROUPS_SIZE}
        onBubbleCountChange={() => {}}
        onGroupsSizeChange={() => {}}
        defaultBubbleCount={150}
      />
    );
  ...
  test("BubbleDonutBuilder onGroupsSizeChange callback should receive all new group sizes after each slider change", () => {
    ...
    const { container } = render(
      <BubbleDonutBuilder
        groupSizes={INITIAL_GROUPS_SIZE}
        onBubbleCountChange={() => {}}
        onGroupsSizeChange={onGroupsSizeChangeCallback}
        defaultBubbleCount={150}
      />
    );

    const slider = container.querySelector(
      'input[name^="group_size"][type=range]'
    ) as Element;
  ...
  test("BubbleDonutBuilder onBubbleCountChange callback should receive new bubble count after bubble count slider change", () => {
    ...
    const { container } = render(
      <BubbleDonutBuilder
        groupSizes={INITIAL_GROUPS_SIZE}
        onBubbleCountChange={onBubbleCountChangeCallback}
        onGroupsSizeChange={() => {}}
        defaultBubbleCount={150}
      />
    );
    ...
    const slider = container.querySelector(
      'input[name="bubble_count"][type=range]'
    ) as Element;
  ...
  test("BubbleDonutBuilder should allow user to add new groups", () => {
    ...
    const addNewGroupButton = container.querySelector(
      'button[name="add_group"]'
    ) as Element;

    fireEvent.click(addNewGroupButton);

    const newExpectedGroupsSize = Math.floor(
      100 / (INITIAL_GROUPS_SIZE.length + 1)
    );
