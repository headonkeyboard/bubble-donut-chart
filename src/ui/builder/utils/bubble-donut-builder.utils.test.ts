...
describe('bubble donut builder utils', () => {
    ...
    test('updatedGroupsSize should return a new array with the updated group size value and the remaining percent equally distributed to other groups', () => {
        ...
        const OTHER_GROUP_SIZE = (100 - NEW_GROUP_SIZE) / (INITIAL_GROUPS_SIZE.length - 1);

        const newGroupsSize = updatedGroupsSize(INITIAL_GROUPS_SIZE, 0, NEW_GROUP_SIZE);
        expect(newGroupsSize).toEqual([NEW_GROUP_SIZE, OTHER_GROUP_SIZE, OTHER_GROUP_SIZE, OTHER_GROUP_SIZE, OTHER_GROUP_SIZE]);
    });
